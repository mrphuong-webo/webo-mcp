...
class SeoArticleAnalysis {
	...
	public static function analyze_tool( array $arguments ) {
		...
		$schema_json_ld = self::build_article_schema_json_ld( $post, is_string( $permalink ) ? $permalink : '', $head_title, $rm );
		$schema_script  = '';
		if ( '' !== $schema_json_ld ) {
	...
	/**
	 * Build a synthetic Article JSON-LD block from WordPress post data.
	 *
	 * @param WP_Post              $post      Post object.
	 * @param string               $permalink Post URL.
	 * @param string               $title     Effective SEO title.
	 * @param array<string, mixed> $rm        Rank Math meta.
	...
	private static function build_article_schema_json_ld( WP_Post $post, string $permalink, string $title, array $rm ) {
		...
		$description = isset( $rm['rank_math_description'] ) && is_string( $rm['rank_math_description'] )
			? wp_strip_all_tags( $rm['rank_math_description'] )
			: '';
		...
		$schema = array(
			...
			'author'           => array(
				'@type' => 'Person',
				'name'  => is_string( $author_name ) && '' !== $author_name ? $author_name : $site_name,
			),
			'publisher'        => array(
				'@type' => 'Organization',
				'name'  => $site_name,
	...
	/**
	 * Estimates word count for space-delimited and non-space-delimited scripts.
	 *
	 * @param string $text       Full text.
	 * @param int    $base_count Regex token count.
	 * @return int
	 */
	private static function estimate_word_count( string $text, int $base_count ) {
		...
		foreach ( $groups as $pattern => $chars_per_word ) {
			if ( preg_match_all( $pattern, $text, $matches ) ) {
				$estimated += (int) floor( count( $matches[0] ) / $chars_per_word );
			}
		}
	...
	private static function detect_seo_issues( array $content, array $structured_data, array $readability ) {
		...
		$title_len  = self::mb_length( $title );
		$meta_len   = self::mb_length( $meta );
		...
		if ( '' === $title ) {
			...
			$issues[] = array(
				'severity' => 'Warning',
				'area'     => 'Title',
				'finding'  => 'Title too short (' . $title_len . ' chars).',
			...
			$issues[] = array(
				'severity' => 'Warning',
				'area'     => 'Title',
				'finding'  => 'Title may be truncated in SERPs (' . $title_len . ' chars).',
		...
		if ( '' === $meta ) {
			...
			$issues[] = array(
				'severity' => 'Warning',
				'area'     => 'Meta Description',
				'finding'  => 'Meta description too short (' . $meta_len . ' chars).',
			...
			$issues[] = array(
				'severity' => 'Warning',
				'area'     => 'Meta Description',
				'finding'  => 'Meta description may be truncated (' . $meta_len . ' chars).',
